...
async function appendToSheet(lead) {
  const range = encodeURIComponent(`${state.sheetTab}!A:I`);
  const res = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${state.spreadsheetId}/values/${range}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS&key=${state.apiKey}`,
    { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ values: [leadToRow(lead)] }) }
  );
...
async function updateRowInSheet(lead) {
  ...
  const range = encodeURIComponent(`${state.sheetTab}!A${lead.rowIndex}:I${lead.rowIndex}`);
  const res = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${state.spreadsheetId}/values/${range}?valueInputOption=USER_ENTERED&key=${state.apiKey}`,
    { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ values: [leadToRow(lead)] }) }
  );
  if (!res.ok) { const e = await res.json(); throw new Error(e.error?.message); }
}
async function deleteRowInSheet(lead) {
  ...
  const range = encodeURIComponent(`${state.sheetTab}!A${lead.rowIndex}:I${lead.rowIndex}`);
  const res = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${state.spreadsheetId}/values/${range}:clear?key=${state.apiKey}`,
