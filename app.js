...
function doLogin(){
  ...
  const client=getClients().find(c=>c.username.toLowerCase()===username.toLowerCase()&&c.password===password&&c.active!==false);
  if(client){sessionStorage.setItem('crm_session',JSON.stringify({role:'client',clientId:client.id}));loadClientCRM(client);return;}
...
function showAdminScreen(){
  hideAllScreens();document.getElementById('admin-screen').classList.add('active');
  ...
  el.innerHTML=clients.map(c=>`
    <div class="client-row">
      <div class="client-row-info" onclick="adminOpenClient('${c.id}')">
        ...
        <div class="client-row-sub">@${esc(c.username)} · לשונית: ${esc(c.sheetTab||'Main CRM')}</div>
      </div>
...
function deleteClient(id){
  ...
  renderAdminClients();renderAdminStats();showAdminToast('לקוח נמחק','success');
}
...
function buildMonthFilter(){
  ...
  sel.innerHTML='<option value="all">כל הזמן</option>'+
    sorted.map(m=>{const[y,mo]=m.split('-');return `<option value="${m}">${HEB_MONTHS[parseInt(mo)-1]+' '+y}</option>`;}).join('');
  if(sorted.length&&selectedMonth!=='all'&&!sorted.includes(selectedMonth))selectedMonth='all';
